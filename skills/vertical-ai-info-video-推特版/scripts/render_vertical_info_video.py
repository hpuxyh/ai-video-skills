...
def grade_image(img, idx, total, cfg=None):
    grade = (cfg or {}).get("image_grade", {})
    img = img.convert("RGB")
    mean_luma = sum(ImageStat.Stat(img.resize((1, 1))).mean) / 3
    if idx == total - 1 or mean_luma < 45:
        img = ImageEnhance.Brightness(img).enhance(float(grade.get("dark_brightness", 1.70)))
        img = ImageEnhance.Contrast(img).enhance(float(grade.get("dark_contrast", 1.22)))
        img = ImageEnhance.Color(img).enhance(float(grade.get("dark_color", 1.06)))
    elif mean_luma < 85:
        img = ImageEnhance.Brightness(img).enhance(float(grade.get("low_brightness", 1.12)))
        img = ImageEnhance.Contrast(img).enhance(float(grade.get("low_contrast", 1.04)))
    elif mean_luma > 210:
        img = ImageEnhance.Brightness(img).enhance(float(grade.get("high_luma_brightness", 0.72)))
        img = ImageEnhance.Contrast(img).enhance(float(grade.get("high_luma_contrast", 0.96)))
    else:
        img = ImageEnhance.Brightness(img).enhance(float(grade.get("normal_brightness", 0.94)))
        img = ImageEnhance.Contrast(img).enhance(float(grade.get("normal_contrast", 0.98)))
    if "brightness" in grade:
        img = ImageEnhance.Brightness(img).enhance(float(grade["brightness"]))
    if "contrast" in grade:
        img = ImageEnhance.Contrast(img).enhance(float(grade["contrast"]))
    if "color" in grade:
        img = ImageEnhance.Color(img).enhance(float(grade["color"]))
    return img


def fit_cover(img, size, idx, total, cfg=None, zoom=1.0):
    target_w, target_h = size
    img = grade_image(img, idx, total, cfg)
...
def fit_contain(img, size, idx, total, zoom_amount, progress, cfg=None, is_pull=False):
    target_w, target_h = size
    img = grade_image(img, idx, total, cfg)
    eased = ease(progress)
...
def photo_layer(img, progress, idx, total, cfg):
    ...
    if photo_fit == "contain":
        bg = cover_crop(grade_image(img, idx, total, cfg).filter(ImageFilter.GaussianBlur(18)), view_size)
        ...
        photo = fit_cover(img, view_size, idx, total, cfg=cfg, zoom=zoom).convert("RGBA")
