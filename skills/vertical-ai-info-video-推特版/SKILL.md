...
- Cover: for 小红书/抖音, make the cover from real people/company assets rather than a pure text card. Reuse the same three animated title lines as the large cover headline, protect the face and full representative subject, add a company logo badge, and show only one conclusion row with no numeric prefix. If there is no suitable product/event person, check the parent company or backing company for a more recognizable representative person or brand asset before using a product page screenshot. If no person is suitable, use the company's logo/product page/official visual and preserve the complete brand/product identity.
